...
public class InMessageShow extends HttpServlet{
	/**
	 * 
	 */
...
//		HttpSession session = request.getSession(false);
		...
		if (method.equals("GET")){
			...
			String message = request.getParameter("inMessages");
			if (message!= null){
				
				dao = DAOimplHibernate.getInstance();
				String result = dao.getInMessage(message);
				
				request.setAttribute("inMessage", result);
				RequestDispatcher view3 = 
						request.getRequestDispatcher("view3.jsp");
